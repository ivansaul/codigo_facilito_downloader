...
Python-based web scraper and downloader for Codigo Facilito courses using **Playwright** for browser automation and **asyncio** for async operations. The project provides both a CLI (`facilito`) and an async API (`AsyncFacilito`).
...
1. **Collectors** (`src/facilito/collectors/`): Scrape course metadata using Playwright selectors

   - `course.py`: Extracts course structure (chapters → units)
   - `unit.py`: Fetches unit-level metadata
   - `video.py`: Extracts video URLs and IDs from page DOM

2. **Downloaders** (`src/facilito/downloaders/`): Handle actual file downloads
   - `course.py`: Orchestrates multi-chapter downloads with directory structure `COURSE_DIR/{NN}_{chapter-slug}/{NN}_{unit-slug}.ext`
...
- **VSD Binary**: Auto-downloaded HLS downloader (v0.3.2), platform-specific, cached in `.bin/` directory
...
- **Pydantic Models**: Defined in `models.py` for type safety (Course, Chapter, Unit, Video, etc.)
